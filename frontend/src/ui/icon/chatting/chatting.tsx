...
const Chatting: React.FC = () => {
  const [activeTab, setActiveTab] = useState('메시지');
        ...
        {activeTab === '메시지' ? (
          ...
          <div className="participants-list">
            {participantDetails.length === 0 ? (
              <p style={{padding: '20px', color: '#888'}}>참여자가 없습니다.</p>
            ) : (
              <>
                <p style={{padding: '20px', color: '#666', fontWeight: 'bold'}}>{participantDetails.length}명이 대화 중입니다.</p>
                <div className="participants-grid">
                  {participantDetails.map((participant) => (
                    <div key={participant.id} className="participant-card">
                      <div className="participant-avatar">
                        {participant.avatar_url ? <img src={participant.avatar_url} /> : "👤"}
                      </div>
